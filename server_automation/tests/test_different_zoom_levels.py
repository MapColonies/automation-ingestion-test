...
def test_zoom_level(zoom_lvl):
    ...
    if zoom_lvl == config.FIRST_ZOOM_LEVEL:
        ...
        result = s3_c.list_objects(Bucket=config.S3_BUCKET_NAME,
                                   Prefix=f'{product_id}/{product_version}/OrthophotoHistory/', Delimiter='/')
        _log.info(f'Got s3 files from product: {product_id} and id :{product_id} , result : {result}')
        try:
        ...
        assert verification_list_0_to_4 == ZOOM_LEVEL_0_TO_4, \
            f'Test: [{test_zoom_level.__name__}] Failed: on validation : [{str(e)}]'
        ...
        assert verification_list_0_to_10 == ZOOM_LEVEL_0_TO_10, \
            f'Test: [{test_zoom_level.__name__}] Failed: on validation : [{str(e)}]'
        ...
        assert verification_list_0_to_16 == ZOOM_LEVEL_0_TO_16, \
            f'Test: [{test_zoom_level.__name__}] Failed: on validation : [{str(e)}]'
    else:
